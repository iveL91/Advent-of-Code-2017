...
def data_input(filename: str) -> Tuple[str, int, Dict[str, Tuple[Tuple[int, int, str], Tuple[int, int, str]]]]:
    ...
    with open(filename) as f:
        ...
        steps_pattern: Pattern = re.compile(
            r"Perform a diagnostic checksum after (\d+) steps.")
        steps: int = int(re.search(steps_pattern, file).group(1))

        dct_pattern: Pattern = re.compile(
            r"In state (\w):\n  If the current value is 0:\n    - Write the value (\d).\n    - Move one slot to the (left|right).\n    - Continue with state (\w).\n  If the current value is 1:\n    - Write the value (\d).\n    - Move one slot to the (left|right).\n    - Continue with state (\w).")
        dct_findall: List[Tuple[str, str, str, str, str, str]
                          ] = re.findall(dct_pattern, file)
        dct = {tup[0]: ((int(tup[1]), direction(tup[2]), tup[3]), (int(
            tup[4]), direction(tup[5]), tup[6])) for tup in dct_findall}
